...
const SingleCourse = () => {
  ...
  const getEnrollStudent = () => {
    ...
    Axios.get(`${REACT_APP.API_URL}/api/coursemain/Get_Enrolled_Student/${EncryptData(parseInt(courseId))}`, {
      headers: {
  ...
  const getCourse = () => {
    ...
    Axios.get(`${REACT_APP.API_URL}/api/coursemain/GetBatchCoursesOnly/${EncryptData(parseInt(batchId))}`, {
      headers: {
        ApiKey: `${REACT_APP.API_KEY}`
      }
    })
          ...
          if (res.data) {
            if (res.data.length !== 0) {
              ...
              if (res.data[0].nTBId === null) {
                ...
                Axios.get(`${REACT_APP.API_URL}/api/coursemain/GetBatchCourseCreatedBy/${EncryptData(parseInt(batchId))}`, {
                  headers: {
                    ApiKey: `${REACT_APP.API_KEY}`
                  }
                })
