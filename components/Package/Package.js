...
const Package = ({ title, tag, col, position }) => {
    ...
    const getPackage = () => {
        ...
        Axios.get(`${REACT_APP.API_URL}/api/package/GetPackageView/${regid}/${courseId}`, {
            headers: {
                ...
                if (res.data.length !== 0) {
                    ...
                    if (maximumItemCart >= (cartitemcnt + 1)) {
                        ...
                        if (localStorage.getItem('userData')) {
                            ...
                            Axios.get(`${REACT_APP.API_URL}/api/promocode/Get_promocode_detail_logout/${cid}/${EncryptData(getamt)}`, {
                                headers: {
                                    ApiKey: `${REACT_APP.API_KEY}`
                                }
                            })
    ...
    const  getCourse = () => {
        ...
        const parts = url.split("/");
        const courseId = parts[parts.length - 1];
        Axios.get(`${REACT_APP.API_URL}/api/coursemain/GetCoursesView/${courseId}`, {
            headers: {
                ApiKey: `${REACT_APP.API_KEY}`
            }
        })
                                ...
                                <div className={'col-lg-4'}>
                                    <div className={'pricing-table'}>
                                        ...
                                        {((isCartItem) && (pid === item.nPkgId)) ? (<Link href='/cart'><button className='btn-lg w-100 btn btn-primary btn-block p-1'>
                                            <span className='align-middle'>Go to cart</span>
                                        </button></Link>) : ((isCartItem) ? ((item.pkgCheckTbl.length > 2) ? ((JSON.parse(item.pkgCheckTbl).map((pitem, pindex) => ((pitem.nPkgId === item.nPkgId) ? <button type="button" key={pindex + 1} className=" btn-lgw-100 btn btn-outline-success btn-block" disabled={true}>Purchased</button> : (((cartItem).find(obj => (obj.cid === item.nCId && obj.pkgId === item.nPkgId)) !== undefined) ? (<NavLink to='/cart'><button className='btn-lg w-100 btn btn-primary btn-block p-1'>
                                            <span className='align-middle'>Go to cart</span>
                                        </button></NavLink>) : (<button type="button" className="btn btn-lg w-100 btn-outline-success btn-block" onClick={() => BuyPackage(item.nPkgId, item.sPackageName)}>Add to cart</button>)))))) : (((cartItem).find(obj => (obj.cid === item.nCId && obj.pkgId === item.nPkgId)) !== undefined) ? <NavLink to='/cart'><button className='w-100 btn btn-primary btn-block p-1'>
                                            <span className='align-middle'>Go to cart</span>
                                        </button></NavLink> : <button type="button" className="w-100 btn-lg btn btn-outline-success btn-block" onClick={() => BuyPackage(item.nPkgId, item.sPackageName)}>Add to cart</button>)) : '')}
                                    </div>
                                </div>
                            </>
                        )
