...
const CourseDetailsTwo = ({ checkMatchCourses }) => {
  ...
  const getCourseModule = () => {
    ...
    Axios.get(`${REACT_APP.API_URL}/api/courseModule/GetModule/${courseId}`, {
      headers: {
        ...
        .then(res => {
          if (res.data) {
            ...
            Axios.get(`${REACT_APP.API_URL}/api/section/GetCourseSummaryAll/${courseId}/${EncryptData(res.data[0]['nMId'])}`, {
              headers: {
                ApiKey: `${REACT_APP.API_KEY}`
              }
            })
