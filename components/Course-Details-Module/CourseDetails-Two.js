...
const CourseDetailsTwo = ({ checkMatchCourses }) => {
  ...
  const getCourseModule = () => {
    ...
    Axios.get(`${REACT_APP.API_URL}/api/courseModule/GetModule/${EncryptData(parseInt(courseId))}`, {
      headers: {
        ...
        .then(res => {
          if (res.data) {
            ...
            Axios.get(`${REACT_APP.API_URL}/api/section/GetCourseSummaryAll/${EncryptData(parseInt(courseId))}/${EncryptData(res.data[0]['nMId'])}`, {
              headers: {
                ApiKey: `${REACT_APP.API_KEY}`
              }
            })
  ...
  const getMID = (mid, title) => {
    ...
    Axios.get(`${REACT_APP.API_URL}/api/section/GetCourseSummaryAll/${EncryptData(parseInt(courseId))}/${EncryptData(mid)}`, {
      headers: {
        ApiKey: `${REACT_APP.API_KEY}`
      }
    })
