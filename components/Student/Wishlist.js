...
const Wishlist = () => {
  ...
  const handleRemoveWishlistItem = (index) => {
    return () => {
      ...
      if (localStorage.getItem('userData')) {
        ...
        Axios.delete(`${REACT_APP.API_URL}/api/wishList/DeleteWishlist/${EncryptData(wishlistitem[index].cid)}/${EncryptData(wishlistitem[index].pkgId)}/${udata['regid']}`, {
          headers: {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('userData')) {
      ...
      Axios.get(`${REACT_APP.API_URL}/api/wishList/GetWishlistItem/${udata['regid']}`, {
        headers: {
          ApiKey: `${REACT_APP.API_KEY}`
        }
      })
          ...
          <div className="content">
            ...
            <div className="row g-5">
              {wishlistitem.map((data, index) => {
                return (
                    <>
                        <div className="col-lg-4 rbt-card variation-01 rbt-hover p-3">
